<template>
  <div class="wrapper-small">
    ...
    <Search />
    <div
      class="my-10 bg-gray-100 p-4 md:p-6 rounded-xl shadow-md"
      v-for="post of posts"
      :key="post.slug"
    >
      <div class="flex justify-between items-center">
        ...
        <span
          class="px-3 py-1 bg-gray-600 text-white text-sm font-bold rounded"
          >{{ post.tag }}</span
        >
      </div>
      ...
      <div class="mt-2">
        <nuxt-link
          :to="post.slug"
          class="text-xl md:text-2xl text-gray-700 font-bold hover:text-gray-600 hover:underline"
          >{{ post.title }}
        </nuxt-link>
        <p class="mt-2 text-base lg:text-lg text-gray-700">
          {{ post.description }}
        </p>
      </div>
